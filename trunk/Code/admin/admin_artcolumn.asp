...
<!--#include file="inc/lib/class_artcolumn.asp"-->
<%
'=========================================================
' File Name：	admin_artcolumn.asp
' Purpose：		文章栏目管理
' Auhtor: 		Foolin
' E-mail: 		Foolin@126.com
' Createed on: 	2009-9-9 10:27:38
...
Sub DoDelete()
	...
	If objC.Delete Then
		...
		Call MsgAndGo("删除栏目[id:"& id &"]成功！", "admin_artcolumn.asp")
	Else
		Call MsgBox("错误：" & objC.LastError, "BACK")
	End If
	Set objC = Nothing
        ...
        <tr>
            <td id="sidebar" valign="top">
                ...
                <ul class="menu">
                 ...
                 <li><a href="admin_article.asp?action=create">添加文章</a></li>
                 <li><a href="admin_article.asp?action=list">管理文章</a></li>
                 <li><a href="admin_article.asp?action=list&list=trash">文章回收站</a></li>
                 <li class="mTitle">--== 文章栏目 ==--</li>
                 <li <%If Request("action") = "create" Then Echo("class=""on""")%>><a href="admin_artcolumn.asp?action=create">添加栏目</a></li>
                 <li <%If Request("action") <> "create" Then Echo("class=""on""")%>><a href="admin_artcolumn.asp">管理栏目</a></li>
                </ul>
				<%Call SysInfo()%>
            </td>
...
%>
	...
	%>
        <tr onMouseOver="this.style.background='#C8E3E2';" onMouseOut="this.style.background='#F0F8FF'">
            ...
            <td>
				<a href="?action=dodelete&id=<%=Rs.Data("ID")%>" onclick="return confirm('将删除该栏目以及该栏目下所有文章！\n\n删除将不能恢复，确定删除该数据？')">删除</a>
            </td>
        </tr>
...
%>
        <tr onMouseOver="this.style.background='#C8E3E2';" onMouseOut="this.style.background='#F0F8FF'" style="color:#666;">
            ...
            <td>
				<a href="?action=dodelete&id=<%=Rs1("ID")%>" onclick="return confirm('将删除该栏目以及该栏目下所有文章！\n\n删除将不能恢复，确定删除该数据？')">删除</a>
            </td>
        </tr>
<%
...
Sub ColForm(ByVal id)
	Dim objA: Set objA = New ClassArtColumn
	If Cint(id) > 0 Then
		objA.ID = id
                ...
                <td>
                	<select name="fParentID">
                    	...
                    	<%If objA.ParentID> 0 Then%>
                    		...
                    		<option value="<%=objA.ParentID%>" selected="selected"> => <%=GetColName(objA.ParentID, "article")%> <= </option>
                        <%Else%>
