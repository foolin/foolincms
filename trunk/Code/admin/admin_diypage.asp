...
Call ChkLogin()	'检查登录
Call ChkPower("diypage","all") '检查权限
Call Init()		'初始化页面
...
<title><%=SITENAME%>后台管理 - 自定义页面管理 - Powered by eekku.com</title>
	...
	%>
        <tr onMouseOver="this.style.background='#C8E3E2';" onMouseOut="this.style.background='#F0F8FF'">
            ...
            <%If Len(Rs.Data("PageName")) > 0 Then%>
                 diypage.asp?url=<%=Rs.Data("PageName")%> [<a href="../diypage.asp?url=<%=Rs.Data("PageName")%>" target="_blank">浏览]</a>
           <%Else%>
                diypage.asp?id=<%=Rs.Data("ID")%> [<a href="../diypage.asp?id=<%=Rs.Data("ID")%>" target="_blank">浏览]</a>
          <%End If%>
            </td>
